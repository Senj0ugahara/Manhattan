...
// Сборка папки pages
function pages() {
  return gulp.src(`${srcFloder}pages/*.html`)
    .pipe(size())
    .pipe(gulp.dest(`${distFloder}pages/`))
    .pipe(browserSync.stream())
}

// Сборка lightbox
function lightbox() {
  const lightboxModules = [
    'node_modules/lightbox2/dist/css/lightbox.min.css',
    'node_modules/lightbox2/dist/js/lightbox-plus-jquery.min.js'
  ]

  return gulp.src(lightboxModules)
    .pipe(gulp.dest(`${distFloder}resources/libraries/`))
}
...
function watch() {
  ...
  gulp.watch(`${distFloder}pages/`, pages);
}

// Настройка gulp build
const build = gulp.series(clean, html, gulp.parallel(styles, scripts, images, resources, pages, lightbox), watch);
